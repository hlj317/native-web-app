...
var UIHeader = require('UIHeader'),
    ...
    UIPageView = require('UIPageView'),
    UIAlert = require('UIAlert'),
    FastClick = require('fastclick');

var AbstractApp = _.inherit({
    propertys: function () {
        //view搜索目录
        ...
        //是否开启单页应用
        //      this.isOpenWebapp = _.getHybridInfo().platform == 'ysbox' ? true : false;
    ...
    setUrlRule: function (viewId, param, replace, project) {
        ...
        for (key in param) {
            _k = encodeURIComponent(_.removeAllSpace(key));
            _v = encodeURIComponent(_.removeAllSpace(param[key]));
            if (i === 0) {
                str += '?' + _k + '=' + _v;
                i++;
            } else {
    ...
    loadView: function (path, callback) {
        var self = this;
        var View = require('../webapp/pages/' + path + '/' + path);
