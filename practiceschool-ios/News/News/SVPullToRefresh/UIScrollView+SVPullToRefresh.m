...
- (void)triggerPullToRefresh {
    if (self.pullToRefreshView.state == SVPullToRefreshStateTriggered
...
//        self.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhite;
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"下拉刷新",@"下拉刷新"),
                             NSLocalizedString(@"松开刷新",@"松开刷新"),
                             NSLocalizedString(@"正在加载...",@"正在加载..."),
                                nil];
...
#pragma mark - avoid animation wrong with insect in IOS7
+(void)avoidAnimationWrong:(UIScrollView *)scrollView ScrollViewTop:(float) top inViewController:(UIViewController *)vc
{
    UIEdgeInsets currentInset = scrollView.contentInset;
    currentInset.top = top;
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, (self.bounds.size.height-32)/2.0, 32, 32)];
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"下拉刷新",@"下拉刷新");
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        imageView.image = [UIImage imageNamed:@"pull_refresh.png"];
    }
    return self;
}
@end
