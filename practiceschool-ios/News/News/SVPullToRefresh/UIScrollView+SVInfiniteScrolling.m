...
- (void)triggerInfiniteScrolling {
    if (self.infiniteScrollingView.state == SVInfiniteScrollingStateTriggered
...
- (void)layoutSubviews {
    ...
    CGSize titleSize = [self.tipLabel.text sizeWithFont:self.tipLabel.font
                                      constrainedToSize:CGSizeMake(labelMaxWidth,self.tipLabel.font.lineHeight)
                                          lineBreakMode:self.tipLabel.lineBreakMode];
    CGFloat totalMaxWidth;
    if (titleSize.width) {
        totalMaxWidth = leftViewWidth + margin + titleSize.width;
    } else {
        totalMaxWidth = leftViewWidth;
    }
    CGFloat labelX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + leftViewWidth + margin;
    CGFloat titleY = (self.bounds.size.height / 2)  - (titleSize.height / 2);
    self.tipLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, titleSize.width, titleSize.height));
    
    CGFloat activityX = (self.bounds.size.width / 2) - (totalMaxWidth / 2);
    self.activityIndicatorView.frame = CGRectMake(activityX,
...
- (UILabel *)tipLabel {
    if(!_tipLabel) {
        _tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 210, 20)];
        _tipLabel.text = NSLocalizedString(@"正在加载...",@"正在加载...");
